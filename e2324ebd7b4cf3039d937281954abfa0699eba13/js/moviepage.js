 ...
 */
onload = onhashchange = function () {  // check if the page is loaded or the url hash changed
    $(".dynamic, footer").stop().slideUp("fast"); // stop current slide animation if present and quickly slide up
...
function changeHash(offset) {                      // parameter can be set to 1 by default with (offset = 1)
    ...
    const range = 4;
    if (range < x || isNaN(x)) {                    // if the result would go beyond the range or if the hash is not a number (it shouldn't happen anyway)
        return location.hash = 1;                   // set the hash to 1 and exit the function
    } else if (x < 1) {                             // if the result it's smaller than the possible IDs
        return location.hash = range;               // set the hash to the last element and exit the function
    }
